...
//  TMC_GetCoordinate(5,1); WaitTime in ms
    ...
    std::vector<std::string> params = GeoCom_Class_Leica::writeAndRead(req);
    //std::cout << "numero de parametros recibidos por la TS: " << params.size() << std::endl;             ///COMENTAR
    return params;
}
/****************************************************************************** TMC_GetSimpleMea *****************************************************************************************/
...
std::vector<std::string> GeoCom_Class_Leica::writeAndRead(std::string cmd)
{
    ...
    //std::cout << "Printing the data received by the Total Station: " << serverReply << std::endl;        ///COMENTAR
